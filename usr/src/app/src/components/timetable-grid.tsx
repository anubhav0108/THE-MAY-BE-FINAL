          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Click any class for LMS options. Use override to make edits.</CardDescription>
              </div>
              <div className="flex flex-wrap gap-2">
                 {isEditing ? (
                    <>
                      <Button variant="outline" onClick={handleCancelChanges}><XCircle className="mr-2 h-4 w-4" /> Cancel</Button>
                      <Button onClick={handleSaveChanges}><Save className="mr-2 h-4 w-4" /> Save Changes</Button>
                    </>
                 ) : (
                    <>
                      {userRole === 'admin' && <Button variant="outline" onClick={handleManualOverride}><Pencil className="mr-2 h-4 w-4" />Manual Override</Button>}
                  ...
                  {timeSlots.map((time) => (
                    <TableRow key={time}>
                      ...
                      {days.map((day) => {
                        ...
                        return (
                          <TableCell key={day} className="p-1 align-top min-w-[220px]">
                            {entry ? (
                              <DialogTrigger asChild>
                              <div 
                                className="p-2 rounded-lg bg-card text-card-foreground border space-y-2 h-full cursor-pointer hover:bg-muted/50 transition-colors"
                              ...
                              >
                                {isEditing ? (
                                    ...
                                    <div className="mt-2 space-y-2">
                                        ...
                                        {suggestionState?.suggestion && (
                                            <Alert>
                                                ...
                                                <AlertTitle className="font-bold text-primary">AI Suggestion</AlertTitle>
                                                <AlertDescription className="text-xs">
                                                    <p className="font-semibold">{suggestionState.suggestion}</p>
                                                    <p className="text-muted-foreground italic mb-2">"{suggestionState.justification}"</p>
                                                    <Button size="sm" className="w-full h-7" onClick={(e) => {e.stopPropagation(); applySuggestion(day, time);}}>
                                                        Apply Suggestion
                                                    </Button>
                                                </AlertDescription>
                                            </Alert>
                                  ...
                                  <>
                                    <p className="font-bold text-sm">{entry.course} ({entry.courseCode})</p>
                                    <p className="text-xs text-muted-foreground">{entry.faculty}</p>
                                    <p className="text-xs font-mono text-muted-foreground">Room: {entry.room}</p>
                                  </>
                                )}
                              </div>
                              </DialogTrigger>
                            ) : (
                                <div className="p-2 text-center text-muted-foreground/50 h-full flex items-center justify-center">
                                    -
                                </div>
                            )}
                          </TableCell>
